...
# based on https://github.com/Tutanchamon/pl_stemmer
...
def stem(word):
    ...
    st = remove_adjective_ends(st)
    st = remove_verbs_ends(st)
    st = remove_adverbs_ends(st)
    st = remove_plural_forms(st)
    st = remove_general_ends(st)
    return st
...
def remove_adjective_ends(word):
    ...
    if len(word) > 5 and word[-3:] in {"owy", "owa", "owe", "ych", "ego"}:
        return word[:-3]
    if len(word) > 5 and word[-2:] in {"ej", "ym", "im"}:
        return word[:-2]
    return word
